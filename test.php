<?php
$link = 'https://drive.google.com/file/d/0B4k5OikQ11AlWlFLaF9hRFJHVk0/view';
...
function Drive($link) {
	...
	foreach($data as $list) {
		$data2 = explode('|', $list);
		if($data2[0] == 37) {$js	.= '{label: "1080p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=luca15/phimconggiao.mp4"},';}
		if($data2[0] == 22) {$js	.= '{label: "720p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=luca15/phimconggiao.mp4"},';}
        if($data2[0] == 59) {$js	.= '{label: "480p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=luca15/phimconggiao.mp4"},';}
		if($data2[0] == 18) {$js	.= '{label: "360p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=luca15/phimconggiao.mp4"},';}
	}
	return rtrim($js, '|');
}
echo "<script src='http://www.domain.com/jwplayer.7.1.js'></script>
	<div id='player'>Đang tải video...</div>
